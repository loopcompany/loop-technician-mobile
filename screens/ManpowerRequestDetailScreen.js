...
import { getManpowerRequestById } from '../services/Api';

export default function ManpowerRequestDetailScreen({ route, navigation }) {
  ...
  const getTypeLabel = (type) => {
    return type === 'field' ? 'نیروی میدانی' : 'نیروی انسانی (داخلی)';
  };

  const getTypeIcon = (type) => {
    return type === 'field' ? 'construct' : 'people';
  };
        ...
        <View style={styles.card}>
          ...
          <View style={styles.header}>
            <View style={styles.titleContainer}>
              <Ionicons name="document-text" size={24} color={themeColor0.bgColor(1)} />
              <Text style={styles.title}>درخواست نیروی انسانی</Text>
            </View>
