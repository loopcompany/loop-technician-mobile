# سیستم چت تکنسین - مستندات پیاده‌سازی

## 📋 خلاصه تغییرات

سیستم چت برای اپلیکیشن تکنسین پیاده‌سازی شد. این سیستم به تکنسینان امکان می‌دهد با کاربرانی که سفارش فعال دارند، ارتباط برقرار کنند.
...
2. **`screens/chat/ChatItem.js`** - تبدیل از نمایش تکنسین به نمایش کاربر:
   - `item.technician` → `item.user`
   - `item.is_read` → `item.unread_count`
   - نمایش "شما: " برای پیام‌های تکنسین
...
4. **`screens/chat/MessegeItem.js`** - تغییر منطق نمایش:
   ...
   - `is_user=0` (پیام تکنسین) → سمت راست
...
✅ نمایش تمام چت‌های تکنسین با کاربران  
...
✅ تمایز بصری بین پیام‌های تکنسین و کاربر  
...
```javascript
{
  ...
  technician_id: 5,
  msg: "متن پیام",
  is_user: 1,              // 1=از کاربر، 0=از تکنسین
  is_read: 0,
  is_closed: 0,
  created_at: "2025-11-08T14:30:25.000000Z"
}
...
- **پیام تکنسین** (سمت راست): `themeColor3.bgColor(0.1)` - خاکستری روشن
...
- [ ] تمایز بصری پیام‌های تکنسین و کاربر
